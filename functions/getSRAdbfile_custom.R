get_sradb_sql_file <- function (destdir = getwd(), destfile = "SRAmetadb.sqlite.gz", 
					method) 
{
	if (missing(method)) 
	...
	if (!inherits(try(url(url_sra_1, open = "rb"), silent = TRUE), 
								"try-error")) {
		url_sra = url_sra_1
	}
	else if (!inherits(try(url(url_sra_2, open = "rb"), silent = TRUE), 
										 "try-error")) {
		url_sra = url_sra_2
	}
	else {
	...
	download.file(url_sra, destfile = localfile, mode = "w", 
								method = method)
	message("Unzipping...\n")
